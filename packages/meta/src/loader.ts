  ...
  for (const src of sources) {
    if (typeof src === "string") {
      try {
        ...
        const definition: NuxtComponentMeta = await import(modulePath).then(
          (m) => m.default || m
        );
        ...
        logger.error(
          `Unable to load static components definitions from "${src}"`,
          error
        );
      }
    } else {
