...
import type { NuxtComponentMetaNames } from "#sse-component-meta/types";

// @ts-ignore
export const __getComponentMeta = async () => {
  const __metas = await import("#sse-component-meta");
  return __metas?.default || __metas;
};

export async function useComponentMeta<T>(
  componentName?: NuxtComponentMetaNames | Ref<NuxtComponentMetaNames>
): Promise<ComputedRef<T extends string ? ComponentData : NuxtComponentMeta>> {
  ...
  // @ts-ignore
  if (process.dev) {
    // Development ; use #sse-component-meta virtual module
    ...
    const { data } = await useAsyncData(
      `sse-component-meta${_componentName ? `-${_componentName}` : ""}`,
      () => {
        return $fetch(
          `/api/component-meta${_componentName ? `/${_componentName}` : ""}`
        );
      }
    );
