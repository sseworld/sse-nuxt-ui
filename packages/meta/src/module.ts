...
import {
  type ComponentMetaParser,
  useComponentMetaParser,
  type ComponentMetaParserOptions,
} from "./parser";
...
export default defineNuxtModule<ModuleOptions>({
  meta: {
    name: "sse-component-meta",
    configKey: "sseComponentMeta",
  },
  defaults: (nuxt) => ({
    ...
    transformers: [
      ...
      (component, code) => {
        ...
        const name =
          code.match(
            /(const|let|var) ([a-zA-Z][a-zA-Z-_0-9]*) = useSlots\(\)/
          )?.[2] || "$slots";
        ...
        const slotNames = code.match(
          /(const|let|var) {([^}]+)}\s*=\s*useSlots\(\)/
        )?.[2];
  ...
  async setup(options, nuxt) {
    ...
    transformers = await nuxt.callHook(
      "component-meta:transformers" as any,
      transformers
    );

    // Resolve loaded components
    let componentDirs: (string | ComponentsDir)[] = [
    ...
    nuxt.hook("components:dirs", (dirs) => {
      ...
      if (uiTemplatesPath) {
        componentDirs.push({
          path: uiTemplatesPath.replace("/index.mjs", "/templates"),
        });
      }
    ...
    addTemplate({
      ...
      getContents: () =>
        [
          "import type { ComponentData } from 'sse-component-meta'",
          `export type NuxtComponentMetaNames = ${[
            ...components,
            ...Object.values(metaSources),
          ]
            .map((c) => `'${c.pascalName}'`)
            .join(" | ")}`,
    ...
    nuxt.options.alias["#sse-component-meta"] = join(
      nuxt.options.buildDir,
      "sse-component-meta.mjs"
    );
    nuxt.options.alias["#sse-component-meta/types"] = join(
      nuxt.options.buildDir,
    ...
    nuxt.hook("nitro:config", (nitroConfig) => {
      ...
      nitroConfig.virtual["#sse-component-meta/nitro"] = () =>
        readFileSync(
