...
export const generate = defineCommand({
  ...
  async setup({ args }) {
    ...
    const inputSource = join(buildDir, "./sse-component-meta.mjs");
    const inputTypes = join(buildDir, "./sse-component-meta.d.ts");

    const outputEsm = join(outputDir, "./sse-component-meta.mjs");
    const outputCjs = join(outputDir, "./sse-component-meta.cjs");
    const outputTypes = join(outputDir, "./sse-component-meta.d.ts");
    ...
    const { loadNuxt, buildNuxt, installModule, logger } = await loadKit(cwd);
    ...
    const nuxt = await loadNuxt({
      ...
      defaultConfig: {
        modules: [
          async (options, nuxt) => {
            const moduleInstalled = nuxt.options?._installedModules?.some(
              (m) => m.meta?.name === pkg.name
            );
            if (moduleInstalled) {
    ...
    const components = await import(inputSource).then(
      (m: any) => m.default || m
    );
    ...
    logger.success("Types generated in", relative(process.cwd(), outputDir));
  },
});
