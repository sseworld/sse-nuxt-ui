...
<template>
  <div>
    <GithubRelease
      v-slot="{ release, refresh }"
      :query="{ owner: qOwner, repo: qRepo, tag }"
    >
      Fetch last release from query:
      <div>Owner: <input v-model="qOwner" type="text" /></div>
      <div>Repo: <input v-model="qRepo" type="text" /></div>
      <div>Tag: <input v-model="tag" type="text" /></div>
