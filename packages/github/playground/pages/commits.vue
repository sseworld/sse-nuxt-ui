...
<template>
  <div>
    <GithubCommits
      v-slot="{ commits, refresh }"
      :query="{ owner: qOwner, repo: qRepo, source: file, date }"
    >
      <div style="margin-bottom: 0.5rem">
        Repo: <input v-model="qOwner" type="text" /> /
      ...
      >
        <a
          :href="`https://github.com/${qOwner}/${qRepo}/commit/${commit.hash}`"
          target="_blank"
        >
