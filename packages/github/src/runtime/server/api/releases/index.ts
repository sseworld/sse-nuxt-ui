...
export default handler(
  async (event: H3Event) => {
    ...
    let releases = (await fetchReleases(query, githubConfig)) as
      | GithubRawRelease
      | GithubRawRelease[];
    ...
    if (moduleConfig.parseContents) {
      if (Array.isArray(releases)) {
        releases = await Promise.all(
          releases.map((release) => parseRelease(release, githubConfig))
        );
      } else {
    ...
    return ((releases as GithubRawRelease[]) || [])
      .sort((a, b) => (a.v !== b.v ? b.v - a.v : a.date - b.date))
      .filter(Boolean);
  },
  {
