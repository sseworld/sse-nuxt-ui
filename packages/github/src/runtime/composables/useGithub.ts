...
export const useGithub = () => {
    ...
    (query: Q): Promise<T> => {
      ...
      if (process.server) {
        ...
        event.node.res.setHeader(
          "x-nitro-prerender",
          [event.node.res.getHeader("x-nitro-prerender"), url]
            .filter(Boolean)
            .join(",")
        );
      }
  ...
  return {
    fetchRepository: _fetch<GithubRepositoryOptions, GithubRepository>(
      "repository"
    ),
    ...
    ): Promise<GithubRawRelease> =>
      _fetch<GithubRepositoryOptions, GithubRawRelease>("releases")({
        ...query,
        last: true,
      } as any),
    fetchContributors: _fetch<GithubContributorsQuery, GithubRawContributor[]>(
      "contributors"
    ),
    fetchFileContributors: _fetch<
      GithubContributorsQuery,
      GithubRawContributor[]
    >("contributors/file"),
