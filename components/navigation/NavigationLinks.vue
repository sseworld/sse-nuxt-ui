<template>
    <div v-if="links?.length" :class="level > 0 ? ui.wrapperLevel : ui.wrapper" v-bind="attrs">
        <ULink v-for="(link, index) in links" :key="index" v-bind="getULinkProps(link)"
            :class="[ui.base, level > 0 && ui.level]" :active-class="ui.active" :inactive-class="ui.inactive"
            @click="link.click">
