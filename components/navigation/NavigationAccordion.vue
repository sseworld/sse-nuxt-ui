<template>
    <UAccordion :key="route.path" :items="(items as AccordionItem[])" :multiple="multiple" :ui="ui" v-bind="attrs">
        <template #default="{ item, open }">
            <ULink :class="[ui.button.base, level > 0 && ui.button.level]" :active-class="ui.button.active"
                ...
                <UIcon v-if="!item.disabled" :name="ui.button.trailingIcon.name"
        ...
        <template v-for="({ label }, index) in links" :key="index" #[label.toLowerCase()]="{ item }">
            <UNavigationTree :links="item.children" :level="level + 1" :default-open="defaultOpen" :multiple="multiple"
                :class="ui.tree" :ui="{ links: ui.links }" />
        </template>
    </UAccordion>
</template>
