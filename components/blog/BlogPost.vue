<template>
    <article :class="ui.wrapper" v-bind="attrs">
        <div v-if="image || $slots.image" :class="ui.image.wrapper">
            <slot name="image">
                <NuxtImg v-bind="typeof image === 'string' ? { src: image, alt: title } : { alt: title, ...image }"
                    :class="ui.image.base" />
            </slot>
        </div>
        ...
        <div :class="ui.container">
            <div :class="ui.inner">
                <NuxtLink v-if="to" :aria-label="ariaLabel" v-bind="nuxtLinkBind" class="focus:outline-none"
                    tabindex="-1">
            ...
            <div v-if="authors?.length || date" :class="ui.authors.wrapper">
                <slot name="authors">
                    <UAvatarGroup v-if="authors?.length">
                        <UAvatar v-for="(author, index) in authors" :key="index" :alt="author.name"
                            :class="ui.authors.avatar.base" v-bind="{ size: ui.authors.avatar.size, ...author.avatar }">
                            <NuxtLink v-if="author.to" v-bind="{ target: '_blank', ...getNuxtLinkProps(author) }"
