<template>
    <div :class="ui.wrapper" v-bind="attrs">
        ...
        <div :class="ui.container">
            <slot>
                <ULink v-for="(link, index) of links" :key="index" v-bind="getULinkProps(link)" :class="ui.base"
                    :active-class="ui.active" :inactive-class="ui.inactive" @click="link.click">
                    <UIcon v-if="link.icon" :name="link.icon" :class="twMerge(ui.icon.base, link.iconClass)" />
                    <UAvatar v-if="link.avatar" v-bind="{ size: ui.avatar.size, ...link.avatar }"
                        :class="twMerge(ui.avatar.base, link.avatarClass)" />

                    <span :class="ui.label">
                        {{ link.label }}

                        <UIcon v-if="link.target === '_blank'" :name="ui.externalIcon.name"
