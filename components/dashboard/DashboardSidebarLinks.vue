<template>
    <component :is="draggable && Container ? Container : 'ul'" :class="ui.wrapper" v-bind="{
        ...(draggable ? { orientation: 'vertical', behaviour: 'contain', lockAxis: 'y', tag: 'ul' } : {}),
        ...attrs
    }" @drag-start="isDragging = true" @drag-end="isDragging = false" @drop="onDrop"
        @touchend="fixActionRestriction">
        <component :is="draggable && Draggable ? Draggable : 'li'" v-for="(link, index) in links" :key="index" tag="li"
            :class="ui.container">
            <component :is="link.children?.length ? Disclosure : 'div'" v-slot="slotProps"
                ...
                <component :is="link.children?.length ? DisclosureButton : 'div'" as="template">
                    <UTooltip class="flex" :popper="{ placement: 'right' }" :prevent="!link.tooltip" :ui="ui.tooltip"
                        v-bind="link.tooltip">
                        <ULink v-slot="{ isActive }"
                            v-bind="link.children?.length && link.collapsible !== false ? { disabled: link.disabled } : getULinkProps(link)"
                            :class="[ui.base, isDragging && 'pointer-events-none']" :active-class="ui.active"
                ...
                <Transition v-bind="ui.transition" @enter="onEnter" @after-enter="onAfterEnter"
                    @before-leave="onBeforeLeave" @leave="onLeave">
                    <DisclosurePanel v-if="link.children?.length && (slotProps?.open || link.collapsible === false)"
                        static as="template">
                        <UDashboardSidebarLinks :level="level + 1" :links="link.children" :draggable="link.draggable"
                            :ui="ui" @update:links="emit('update:links', $event)">
