...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    if (options.defaultTimezone && !options.plugins.includes("timezone"))
      throw new Error(
        "You must include the timezone plugin in order to set a default timezone"
      );
    ...
    addImports({
      name: "useDayjs",
      as: "useDayjs",
      from: nuxt.options.alias["#sse-nui/dayjs"],
    });

    addTemplate({
      filename: "sse-dayjs.imports.mjs",
    ...
    nuxt.hook("prepare:types", ({ references }) => {
      ...
      if (options.externalPlugins) {
        const externalPlugins = options.externalPlugins.map((p) => ({
          types: p.package,
        }));
...
// Generated by sse-dayjs-module
import dayjs from 'dayjs'
import updateLocale from 'dayjs/plugin/updateLocale'

${locales?.map((locale) => `import 'dayjs/locale/${locale}'`).join("\n")}
${plugins
  ?.map((plugin) => `import ${plugin} from 'dayjs/plugin/${plugin}'`)
  .join("\n")}
${externalPlugins
...
${externalPlugins
  ?.map(
    (plugin) => `dayjs.extend(${plugin.name}, ${JSON.stringify(plugin.option)})`
  )
  .join("\n")}
...
dayjs.locale('${
        typeof defaultLocale === "string" ? defaultLocale : defaultLocale[0]
      }')
`
    : ""
}
