...
export function cognitoEventHandler({
  config,
  onSuccess,
  onError,
}: OAuthConfig<OAuthCognitoConfig>) {
  return eventHandler(async (event: H3Event) => {
    ...
    if (
      ...
      const error = createError({
        statusCode: 500,
        message:
          "Missing NUXT_OAUTH_COGNITO_CLIENT_ID, NUXT_OAUTH_COGNITO_CLIENT_SECRET, NUXT_OAUTH_COGNITO_USER_POOL_ID, or NUXT_OAUTH_COGNITO_REGION env variables.",
      });
    ...
    const tokens: any = await $fetch(tokenURL as string, {
      ...
      body: `grant_type=authorization_code&client_id=${
        config.clientId
      }&client_secret=${config.clientSecret}&redirect_uri=${
        parsePath(redirectUrl).pathname
      }&code=${code}`,
    }).catch((error) => {
      return { error };
    });
    ...
    const user: any = await $fetch(
      `https://${config.userPoolId}.auth.${config.region}.amazoncognito.com/oauth2/userInfo`,
      {
        headers: {
