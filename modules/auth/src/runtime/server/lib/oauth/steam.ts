...
export function steamEventHandler({
  config,
  onSuccess,
  onError,
}: OAuthConfig<OAuthSteamConfig>) {
  return eventHandler(async (event: H3Event) => {
    ...
    if (!query["openid.claimed_id"]) {
      ...
      return sendRedirect(
        event,
        withQuery(config.authorizationURL as string, steamOpenIdParams)
      );
    }

    // Validate OpenID Authentication
    const validateAuth: string = await $fetch(
