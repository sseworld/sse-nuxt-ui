...
export function spotifyEventHandler({
  config,
  onSuccess,
  onError,
}: OAuthConfig<OAuthSpotifyConfig>) {
  return eventHandler(async (event: H3Event) => {
    ...
    if (!config.clientId || !config.clientSecret) {
      const error = createError({
        statusCode: 500,
        message:
          "Missing NUXT_OAUTH_SPOTIFY_CLIENT_ID or NUXT_OAUTH_SPOTIFY_CLIENT_SECRET env variables.",
      });
