...
export function microsoftEventHandler({
  config,
  onSuccess,
  onError,
}: OAuthConfig) {
  return eventHandler(async (event: H3Event) => {
    ...
    if (!config.clientId || !config.clientSecret || !config.tenant) {
      const error = createError({
        statusCode: 500,
        message:
          "Missing NUXT_OAUTH_MICROSOFT_CLIENT_ID or NUXT_OAUTH_MICROSOFT_CLIENT_SECRET or NUXT_OAUTH_MICROSOFT_TENANT env variables.",
      });
    ...
    if (!code) {
      const scope =
        config.scope && config.scope.length > 0 ? config.scope : ["User.Read"];
