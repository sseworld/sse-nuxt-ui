...
export function xsuaaEventHandler({
  config,
  onSuccess,
  onError,
}: OAuthConfig<OAuthXSUAAConfig>) {
  return eventHandler(async (event: H3Event) => {
    config = defu(
      config,
      useRuntimeConfig(event).oauth?.xsuaa
    ) as OAuthXSUAAConfig;
    ...
    if (!config.clientId || !config.clientSecret || !config.domain) {
      const error = createError({
        statusCode: 500,
        message:
          "Missing NUXT_OAUTH_XSUAA_CLIENT_ID or NUXT_OAUTH_XSUAA_CLIENT_SECRET or NUXT_OAUTH_XSUAA_DOMAIN env variables.",
      });
