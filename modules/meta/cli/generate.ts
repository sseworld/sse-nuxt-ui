        ...
        modules: [
          async (options, nuxt) => {
            const moduleInstalled = nuxt.options?._installedModules?.some(
              (m) => m.meta?.name === "sse-nui/component-meta"
            );
            if (moduleInstalled) {
              logger.info(`Module "sse-nui/component-meta" already installed`);
              return;
            }

            const module = await import("../index").then((m) => m.default);
