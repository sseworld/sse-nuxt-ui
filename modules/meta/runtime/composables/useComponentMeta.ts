...
import type { NuxtComponentMetaNames } from "#sse-nui/component-meta/types";

// @ts-ignore
export const __getComponentMeta = async () => {
  const __metas = await import("#sse-nui/component-meta");
...
export async function useComponentMeta<T>(
  ...
  if (process.dev) {
    ...
    return computed(
      () =>
        nuxtApp._componentMeta as T extends string
          ? ComponentData
          : NuxtComponentMeta
    );
  } else {
